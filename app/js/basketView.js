...
export default class BasketView {
  ...
  init() {
    this.renderCount(this.model.getItems().length);
    this.model.setDataChangeHandler(this.onDataChange.bind(this));

    this.basketSmall.addEventListener(
      "click",
  ...
  renderCount(count) {
    this.basketSmall.querySelector("span").textContent = count;
  }
  ...
  showBasketDetail(evt) {
    ...
    const cards = this.model.getItems();
    if (cards.length > 0) {
